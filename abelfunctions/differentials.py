...
def mnuk_conditions(g, b, generic_adjoint):
    ...
    denom = b.denominator()
    u,v = map(T, R.gens())
    expr = numer(v,u) * generic_adjoint(v,u)
    ideal = T.ideal(g(v,u))
    r = expr.reduce(ideal)

    # determine the multiplicity of "u=0" in the denominator
    u,v = R.gens()
    p = R(denom).polynomial(u)
    mult = min(p.exponents())
